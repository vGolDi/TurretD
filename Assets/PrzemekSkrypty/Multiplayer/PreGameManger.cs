...
public class PreGameManager : MonoBehaviourPunCallbacks
{
    ...
    private const string PREGAME_START_TIME_KEY =
        "PreGameStartTime";
    ...
    {
        preGameUI =
            FindFirstObjectByType<PreGameUI>();
        preGameUI?.Hide();
    }
    ...
    {
        if (props.ContainsKey(PREGAME_START_TIME_KEY))
        {
            double startTime =
                (double)props[PREGAME_START_TIME_KEY];

            if (!preGamePhaseActive)
            {
                Debug.Log(
                    "[PreGameManager] PreGame start");
                StartCoroutine(
                    PreGameSequence(startTime));
            }
        }

        if (props.ContainsKey(ALL_DECKS_READY_KEY) &&
            (bool)props[ALL_DECKS_READY_KEY])
        {
            if (waitingForOthers)
            {
                Debug.Log(
                    "[PreGameManager] All ready!");
                StartGameCountdown();
            }
        }
    }
        ...
        ExitGames.Client.Photon.Hashtable props)
    {
        if (PhotonNetwork.IsMasterClient &&
            props.ContainsKey(DECK_SELECTED_KEY))
        {
        ...
        double networkStartTime)
    {
        ...
        if (PhotonNetwork.CurrentRoom.CustomProperties
            .TryGetValue(ARENA_TYPE_KEY,
                out object arenaObj))
        {
            preGameUI?.SetArenaType(
                (string)arenaObj);
        }

        double endTime =
            networkStartTime + deckSelectionTime;

        while (PhotonNetwork.Time < endTime &&
               !deckWasSelected)
        {
            float remaining =
                (float)(endTime - PhotonNetwork.Time);
            preGameUI?.UpdateTimer(remaining);
            yield return null;
        }
    ...
    /// <summary>
    /// Called by PreGameUI when player clicks a deck
    /// </summary>
    public void OnDeckSelectedFromUI(DeckData deck)
    {
        OnDeckSelected(deck);
    }
    ...
    private void AutoSelectDeck()
    {
        ...
        var decks =
            PlayerCollection.Instance?.GetPlayerDecks();

        if (decks != null && decks.Count > 0)
        {
            preGameUI?.AutoSelectFirstDeck();
            OnDeckSelected(decks[0]);
        }
        else
        {
            DeckData[] resDecks =
                Resources.LoadAll<DeckData>("Decks");
            if (resDecks.Length > 0)
                OnDeckSelected(resDecks[0]);
            else
                MarkDeckAsSelected();
        }
    }
    ...
    {
        preGameUI?.ShowWaiting();
        preGameUI?.SetWaitingText(
