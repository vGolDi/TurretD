...
{
    /// <summary>
    /// Lootbox reward popup — UI Toolkit version.
    /// Shows when player receives a lootbox from
    /// quests, level-ups, etc.
    /// </summary>
    [RequireComponent(typeof(UIDocument))]
    public class LootboxRewardPopup : MonoBehaviour
    {
        public static LootboxRewardPopup Instance
        { get; private set; }

        [Header("Audio")]
        [SerializeField] private AudioClip rewardSound;
        [SerializeField]
        private AudioClip legendaryRewardSound;
        ...
        private VisualElement rewardGlow;
        private Label lootboxNameLabel;
        ...
        private Queue<RewardQueueItem> rewardQueue =
            new Queue<RewardQueueItem>();
        ...
        private void QueryElements()
        {
            ...
            rewardIcon =
                root.Q<VisualElement>("reward-icon");
            rewardGlow =
                root.Q<VisualElement>("reward-glow");
            lootboxNameLabel =
                root.Q<Label>("reward-lootbox-name");
            reasonLabel =
                root.Q<Label>("reward-reason");
            btnOpen =
                root.Q<Button>("btn-reward-open");
            btnOk =
                root.Q<Button>("btn-reward-ok");
        }
        ...
        private void BindButtons()
        {
            ...
            btnOpen?.RegisterCallback<ClickEvent>(
                evt =>
                {
                    OpenLootboxNow();
                    evt.StopPropagation();
        ...
        private void SubscribeToEvents()
        {
            ...
            {
                LootboxRewardGiver.Instance
                    .OnLootboxRewarded -=
                    OnLootboxRewardedWithReason;
                LootboxRewardGiver.Instance
                    .OnLootboxRewarded +=
                    OnLootboxRewardedWithReason;

                Debug.Log(
                    "[LootboxRewardPopup] Subscribed " +
                    "to LootboxRewardGiver");
            }
            else
            {
                Debug.LogWarning(
                    "[LootboxRewardPopup] " +
                    "LootboxRewardGiver not found!");
            }
        }
        ...
        private void OnLootboxRewardedWithReason(
            LootboxData lootbox, string reason)
        {
            Debug.Log(
                $"[LootboxRewardPopup] Received: " +
                $"{lootbox?.lootboxName} - {reason}");
            ...
            LootboxData lootbox, string reason)
        {
            ...
            Debug.Log(
                $"[LootboxRewardPopup] Queued: " +
                $"{lootbox.lootboxName}. " +
                $"Queue: {rewardQueue.Count}");
        ...
        public void ShowPopup(
            LootboxData lootbox, string reason)
        {
            if (lootbox == null || popupRoot == null)
            {
                ShowNextReward();
                return;
            }
            ...
            // Icon
            if (rewardIcon != null &&
                lootbox.lootboxIcon != null)
            {
                rewardIcon.style.backgroundImage =
                    new StyleBackground(
                        lootbox.lootboxIcon);
            }
            ...
            {
                Color glowColor =
                    lootbox.GetRarityColor();
                glowColor.a = 0.15f;
                rewardGlow.style.backgroundColor =
                    new StyleColor(glowColor);
            }

            // Name
            if (lootboxNameLabel != null)
                lootboxNameLabel.text =
                    lootbox.lootboxName;
            ...
            Debug.Log(
                $"[LootboxRewardPopup] Showing: " +
                $"{lootbox.lootboxName}");
        }

        public void HidePopup(
            bool closeQuestPanel = false)
        {
            if (autoHideCoroutine != null)
            {
                StopCoroutine(autoHideCoroutine);
        ...
        private void OpenLootboxNow()
        {
            ...
            // Find LootboxUI and open
            LootboxUI lootboxUI =
                FindFirstObjectByType<LootboxUI>(
                    FindObjectsInactive.Include);

            if (lootboxUI != null)
            {
                // Navigate to lootbox panel via
                // MainMenuController
                ...
                if (mainMenu != null)
                    mainMenu.OpenLootboxMenu();

                StartCoroutine(
                    DelayedOpenLootbox(
                        lootboxUI, lootboxToOpen));
            }
            else
            {
                Debug.LogWarning(
        ...
        private IEnumerator DelayedOpenLootbox(
            LootboxUI ui, LootboxData lootbox)
        {
            yield return new WaitForSecondsRealtime(
                0.3f);
        ...
        [ContextMenu("Test Show Common")]
        private void TestShowCommon()
        {
            var lootboxes =
                Resources.LoadAll<LootboxData>(
                    "Lootboxes");
            var common = System.Array.Find(
                lootboxes,
                l => l.rarity == LootboxRarity.Common);
            if (common != null)
                QueueReward(common, "Test Reward!");
        }

        [ContextMenu("Test Show Legendary")]
        private void TestShowLegendary()
        {
            var lootboxes =
                Resources.LoadAll<LootboxData>(
                    "Lootboxes");
            var legendary = System.Array.Find(
                lootboxes,
                l => l.rarity ==
                    LootboxRarity.Legendary);
            if (legendary != null)
                QueueReward(
                    legendary, "LEGENDARY Test!");
        }
    }
}
...
//        {
//            // Stare UGUI:
//            // if (QuestPanelSlider.Instance != null &&
//            //     QuestPanelSlider.Instance.IsOpen)
